...
public class FileController {
    ...
    private void createExcelFile(String filePath) throws IOException {
        ...
        XSSFSheet spreadsheet = workbook.createSheet("Drug Data");
        XSSFRow row;
        Map<String, Object[]> drugData = new TreeMap<String, Object[]>();
        drugData.put("1", new Object[]{"Name", "Count", "Type"});
        int i = 1;
        for (Drug drug : repository.findAll()) {
            drugData.put(String.valueOf(i + 1), new Object[]{drug.getName(), drug.getCount(), drug.getType()});
            i++;
        }
        Set<String> keyid = drugData.keySet();
        int rowid = 0;
        for (String key : keyid) {
            row = spreadsheet.createRow(rowid++);
            Object[] objectArr = drugData.get(key);
            int cellid = 0;
    ...
    private void createPDFFile(String filePath) throws IOException {
        ...
        try {
            ...
            doc.open();
            String text = "Report\n" +
                    "from 07/28/23 to 08/08/23 drugs have a rest\n";
            ...
            table.addCell(cell3);
            for (Drug drug : repository.findAll()) {
                PdfPCell cellName = new PdfPCell(new Phrase(drug.getName()));
                PdfPCell cellCount = new PdfPCell(new Phrase(String.valueOf(drug.getCount())));
                PdfPCell cellType = new PdfPCell(new Phrase(drug.getType()));
